...
<!--dir="ltr"は左から右に読む言語との宣言-->
  ...
  if($res<$end_date){
    for ($i=1; $i <=$end_date ; $i++) {
      ...
      $year=$d->format('Y');
      $month=$d->format('m');
      $day_max = intval($pdo->query("SELECT max(day) FROM YJFX where '" . $current_month . "' ")->fetchColumn());
      $day=$day_max+1;
    ...
    for ($j=1; $j <$cnt2 ; $j++) {//$jを1から始めることで列名の配列をループしない
      ...
      if($split3[0]<=$current_date){
        if($i>=1 && $i<=5){
          ...
          $buy4s=$split3[11];
          $sell4s=$split3[12];
          $sql="UPDATE YJFX set updated_at='$updated_at',{$sp1}='$sp1s',{$buy1}='$buy1s',{$sell1}='$sell1s',{$sp2}='$sp2s',{$buy2}='$buy2s',{$sell2}='$sell2s',{$sp3}='$sp3s',{$buy3}='$buy3s',{$sell3}='$sell3s',{$sp4}='$sp4s',{$buy4}='$buy4s',{$sell4}='$sell4s' WHERE day='$days'";
        ...
        if($i==6){
          ...
          $buy2s=$split3[5];
          $sell2s=$split3[6];
          $sql="UPDATE YJFX set updated_at='$updated_at',{$sp1}='$sp1s',{$buy1}='$buy1s',{$sell1}='$sell1s',{$sp2}='$sp2s',{$buy2}='$buy2s',{$sell2}='$sell2s' WHERE day='$days'";
          $stmt=$pdo->prepare($sql);
          $stmt->execute();
        }
      }
